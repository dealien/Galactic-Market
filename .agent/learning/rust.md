...
## 2026-09-13 - Lazy Evaluation of EMA Price Lookup in Merchant AI Hot Path
**Learning:** In heavily nested loops like `compute_merchant_opportunities` (O(R * C * C)), executing an O(1) `HashMap` lookup (like getting an EMA price) for every iteration incurs significant overhead, even if the value isn't always used. Deferring the lookup until *after* boolean gate checks (like verifying the merchant has inventory) drastically reduces hashing overhead.
**Action:** Always order condition checks in hot paths by computational expense (lazy evaluation). Perform simple boolean checks or pointer comparisons first, and defer any operations involving allocations, hashing, or `HashMap` lookups until they are strictly required.
