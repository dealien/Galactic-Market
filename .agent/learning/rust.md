...
## 2024-05-18 - Optimized HashMap Lookup and Tick Loop Overhead
**Learning:** Found redundant map lookups inside a hot path loop across tick iteration (e.g. `markets.rs` when updating inventories and in `decisions.rs` updating a map using entry). In rust `.or_insert` always evaluates the default expression and causes allocation overhead if passing `Vec::new()` or a struct, so `.or_insert_with(|| ...)` should always be used.
**Action:** Replaced `.or_insert(Vec::new())` and struct initialization with `.or_insert_with(|| ...)` in `decisions.rs` and `markets.rs`.
