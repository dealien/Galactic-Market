...
## 2024-05-18 - Struct Setup in Tests
**Learning:** When asserting properties inside complex structs within the `galactic-market` test setup, it's safer to avoid creating the structs from scratch to prevent `missing fields` compilation errors as structs evolve.
**Action:** Always prefer modifying pre-existing state set up by helper functions like `setup_political_state()` or initializing just what is necessary in an empty `SimState::new()` and then adding the exact required testing variables, rather than manually building full domain structs like `City` or `StarSystem` if they have many defaults.
