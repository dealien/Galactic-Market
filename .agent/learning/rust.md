...
## 2025-05-24 - Avoid Complex Types in HashMap Declarations
**Learning:** Explicitly typing complex nested collections like `std::collections::HashMap<i32, (Vec<(i32, i64, f64)>, f64)> = std::collections::HashMap::with_capacity(32)` triggers `clippy::type-complexity` warnings, failing CI.
**Action:** Let Rust type inference resolve complex collection types automatically by simply using `let mut map = std::collections::HashMap::with_capacity(32);` when possible.
