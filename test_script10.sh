cat << 'INNER_EOF' > update_markets.py
import re

with open('src/sim/markets.rs', 'r') as f:
    content = f.read()

pattern = r'''            let \(buy_qty, buy_price, buy_kind, buy_company_id\) = \{
...
replace = '''            let (buy_qty, buy_price, buy_is_limit, buy_company_id) = {
                let o = &state.market_orders[&b_id];
...
pattern2 = r'''            // Check price compatibility for Limit vs Limit
            ...
            let clearing_price = match \(buy_kind\.as_str\(\), sell_kind\.as_str\(\)\) \{
                \("market", "market"\) => \{
                    ...
                    state
                        \.ema_prices
                        \.get&\(\(city_id, resource_id\)\)
                        \.copied\(\)
                        \.unwrap_or\(10\.0\)
                \}
                \("market", "limit"\) => sell_price,
                \("limit", "market"\) => buy_price,
                \("limit", "limit"\) => \(buy_price \+ sell_price\) / 2\.0,
                _ => \(buy_price \+ sell_price\) / 2\.0,
            \};'''
...
replace2 = '''            // Check price compatibility for Limit vs Limit
            ...
            let clearing_price = match (buy_is_limit, sell_is_limit) {
                ...
                (true, true) => (buy_price + sell_price) / 2.0,
            };'''
